"""Generate the continuous PBR0110 visual wrap, with no artificial panel gaps.
...
def generate(recipe):
    ...
    for side,face in enumerate(recipe['faces']):
        ...
        for half,(left,right) in enumerate(((l+inset,r-inset),)):
            ...
            def vertex(i,j):
                ...
                # Continuous box-wrap UVs across vertical corners; no UV reset at face midpoints.
                wrap=face.get('uv_wrap')
                if wrap:
                    texcoords.append([(wrap['offset_m']+wrap['u_sign']*(u-wrap['u_origin_m']))*recipe['atlas_scale_per_m'],(v-wrap['v_origin_m'])*recipe['atlas_scale_per_m']])
                else:
                    texcoords.append([(u-a)*recipe['atlas_scale_per_m'],(v-c)*recipe['atlas_scale_per_m']])
                return n
            ...
            pieces.append(dict(name=face['name'],face=face['name'],bounds_uv_m=list(bounds),vertex_start=offset,front_vertex_count=count,face_start=len(faces)-len(local_faces),front_triangle_count=front_count,triangle_count=len(local_faces),keepouts_m=boxes,uv_island=index))
...
def export(recipe,output):
    ...
    lines=['# Visual-only MLI shell, UE centimetres; recipe is metres; no physical contacts','mtllib SM_SarmMLI_PBR0110.mtl','o SM_SarmMLI_PBR0110']
